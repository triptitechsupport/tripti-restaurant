...
// Minimal header for KDS-authenticated pages only.
...
//
// Logo click: if there is a valid KDS session (or an admin session, since
// admins may also view the KDS Dashboard), navigate to the KDS Dashboard;
// otherwise navigate to the KDS Login page.
//
// This mirrors the WaiterHeader pattern (WaiterHeader.jsx) so the two
// staff surfaces share the same responsive, minimal chrome.
export default function KdsHeader() {
  ...
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        ...
        console.error('[KdsHeader] Failed to load site branding logo:', err);
      }
    };
    fetchLogo();
  }, []);
  ...
  const handleLogoClick = () => {
    const kModel = pb.authStore.model || pb.authStore.record;
    const aModel = pbDefault.authStore.model || pbDefault.authStore.record;
    const isKds = pb.authStore.isValid && kModel?.collectionName === 'kds_users';
      ...
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-[var(--nav-height-mobile)] md:h-[var(--nav-height-desktop)] items-center justify-between gap-4">
          ...
          <button
            ...
            aria-label="Tripti Genusswelt — go to Kitchen Display"
          >
            {logoUrl ? (
              <motion.div
