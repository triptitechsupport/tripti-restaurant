...
export default function FiscalReceiptDialog({ transaction, onClose }) {
  ...
  function printReceipt() {
    ...
    const cleanup = () =>
      document.body.classList.remove("printing-fiscal-receipt");
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
        <DialogHeader className="fiscal-no-print">
          <DialogTitle>
            {t.isFallback ? "Outage receipt preview" : "Receipt preview"}
          </DialogTitle>
          <DialogDescription>
            {t.isFallback
              ? "Prints the saved outage copy. Signing is retried automatically; this copy remains available afterward."
              : "Prints the saved receipt. Reprints do not generate another receipt."}
          </DialogDescription>
        </DialogHeader>
        <div
          className="fiscal-receipt notranslate"
          id="fiscal-receipt"
          translate="no"
        >
          {response._env !== "LIVE" && (
            <div className="fiscal-test-notice">
              TEST RECEIPT
              <br />
              NOT FOR ACCOUNTING
            </div>
          ...
          <section className="fiscal-meta">
            {t.isFallback ? (
              ...
              <ReceiptRow label="Receipt No.">
                <strong>{t.fiskalyReceiptNumber}</strong>
              </ReceiptRow>
            )}
          ...
          <hr />
          <section>
            <div className="fiscal-row fiscal-caption">
              <span>ITEM / ARTIKEL</span>
              <span>EUR</span>
            </div>
            {(snapshot.lines || []).map((line, i) => (
              <div key={i} className="fiscal-item">
                <div className="fiscal-item-name">{line.text}</div>
                <ReceiptRow
                  label={`${line.quantity} × ${money(line.price_per_unit)} · ${line.vatPercent}% VAT`}
                >
                  {money(
                    (Math.round(Number(line.price_per_unit) * 100) *
          ...
          <section className="fiscal-vat">
            <div className="fiscal-row fiscal-caption">
              <span>VAT / MWST.</span>
              <span>GROSS / BRUTTO</span>
            </div>
            {(snapshot.vatTotals || []).map((entry) => {
              const line = (snapshot.lines || []).find(
                (item) => item.vatRate === entry.vat_rate,
              );
              return (
                <ReceiptRow
                  key={entry.vat_rate}
                  label={
                    line?.vatPercent != null
                      ? `${line.vatPercent}%`
                      : entry.vat_rate
                  }
                >
          ...
          {snapshot.cancellationReason && (
            <p className="fiscal-notice">
              Cancellation reason: {snapshot.cancellationReason}
            </p>
          )}
          {(response.hints || []).map((hint, i) => (
            <p key={i} className="fiscal-notice">
              {hint}
            </p>
          ))}
          <hr />
          {!t.isFallback && (
            <ReceiptRow label="Cash register">
              {response.cash_register_serial_number}
            </ReceiptRow>
          )}
          {printable && (
          ...
          <footer className="fiscal-footer">
            <p>
              {t.isFallback
                ? "Outage receipt / Ausfallbeleg"
                : "Fiscal Receipt / Fiskalbeleg"}
            </p>
            <strong>
              {t.isFallback
